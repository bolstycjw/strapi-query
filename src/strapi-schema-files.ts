...
import type { StrapiSchemaDocument } from './strapi-schema.js';

const IGNORED_DIRECTORIES = new Set(['.git', 'dist', 'node_modules']);

export async function readStrapiSchemaFiles(paths: readonly string[]): Promise<StrapiSchemaDocument[]> {
...
async function resolveSchemaFiles(path: string): Promise<string[]> {
...
function isStrapiSchemaFile(path: string) {

...
export interface ResourceDefinition<Entity, Relations extends RelationDefinitions, Kind extends 'collection' | 'single'> {
    kind: Kind;
    path: string;
    relations: Relations;
    readonly __entity?: Entity;
}
export type CollectionResource<Entity, Relations extends RelationDefinitions = {}> = ResourceDefinition<Entity, Relations, 'collection'>;
export type SingleResource<Entity, Relations extends RelationDefinitions = {}> = ResourceDefinition<Entity, Relations, 'single'>;
...
export type CollectionKey<Schema extends SchemaDefinition> = {
    [Key in keyof Schema]: Schema[Key] extends ResourceDefinition<unknown, RelationDefinitions, 'collection'> ? Key : never;
}[keyof Schema];
export type SingleKey<Schema extends SchemaDefinition> = {
    [Key in keyof Schema]: Schema[Key] extends ResourceDefinition<unknown, RelationDefinitions, 'single'> ? Key : never;
}[keyof Schema];
...
type FilterValue<Value> = Value extends Primitive ? Value | Value[] | FieldOperators<NonNullable<Value>> : Value extends Array<infer Item> ? Item | Item[] | FieldOperators<Item> : Value | FieldOperators<Value>;
...
type RelationFilters<Schema extends SchemaDefinition, Key extends keyof Schema> = {
    [RelationKey in StringKeyOf<Relations<Schema, Key>>]?: Filters<Schema, TargetKey<Schema, Relations<Schema, Key>[RelationKey]>>;
};
export type Filters<Schema extends SchemaDefinition, Key extends keyof Schema> = ((FieldFilters<Schema, Key> & RelationFilters<Schema, Key>) | {
    $and?: Filters<Schema, Key>[];
    $or?: Filters<Schema, Key>[];
    $not?: Filters<Schema, Key>;
}) & {};
type PopulateRelationValue<Schema extends SchemaDefinition, Relation extends RelationDefinition> = true | {
...
export type PublicationFilter = 'never-published' | 'has-published-version' | 'modified' | 'unmodified' | 'never-published-document' | 'has-published-version-document' | 'published-without-draft' | 'published-with-draft';
export interface Query<Schema extends SchemaDefinition, Key extends keyof Schema, PopulateShape extends Populate<Schema, Key> | undefined = undefined> {
...
export interface StrapiCollectionClient<Schema extends SchemaDefinition, Key extends CollectionKey<Schema>> {
    findMany<const PopulateShape extends Populate<Schema, Key> | undefined = undefined>(query?: Query<Schema, Key, PopulateShape>): Promise<StrapiCollectionResponse<Populated<Schema, Key, NonNullable<PopulateShape>>>>;
    findFirst<const PopulateShape extends Populate<Schema, Key> | undefined = undefined>(query?: Query<Schema, Key, PopulateShape>): Promise<Populated<Schema, Key, NonNullable<PopulateShape>> | null>;
}
export interface StrapiSingleClient<Schema extends SchemaDefinition, Key extends SingleKey<Schema>> {
    find<const PopulateShape extends Populate<Schema, Key> | undefined = undefined>(query?: Query<Schema, Key, PopulateShape>): Promise<StrapiSingleResponse<Populated<Schema, Key, NonNullable<PopulateShape>>>>;
}
